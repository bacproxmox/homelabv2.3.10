...
monitors=[
 ...
 ('Chia Daemon','port','','192.168.50.107',55400,0),
]
...
for name,mtype,url,host,port,ignore_tls in monitors:
    ...
    data={};
    for k,v in [('user_id',uid),('name',name),('type',mtype),('url',url),('hostname',host),('port',port),('method','GET'),('interval',60),('retryInterval',60),('maxretries',3),('active',1),('ignoreTls',ignore_tls),('upsideDown',0),('maxredirects',10),('accepted_statuscodes','["200-299","300-399","401","403"]'),('created_date',now),('weight',2000)]: set_col(data,mc,k,v)
    if existing:
