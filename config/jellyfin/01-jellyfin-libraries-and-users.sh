...
echo
echo "🎬 Homelab v2.3.10 - Jellyfin Post-Setup Configurator"
echo
...
# v2.3.10: Jellyfin viewer users use passwords collected in users.env.
...
# v2.3.10: Jellyfin server name should be stable and branded, not random/container-derived.
echo
...
if [[ -n "${ORHAN_PASS:-}" ]]; then
  VIEWERS+=("Orhan")
  VIEWER_PASSWORDS+=("$ORHAN_PASS")
else
  echo "ℹ️ ORHAN_PASS tanımlı değil; Orhan kullanıcısı otomatik oluşturulmayacak."
fi
...
echo "  ATLON_PASS / ELIFEZEL_PASS / TULUMBA_PASS"
echo
