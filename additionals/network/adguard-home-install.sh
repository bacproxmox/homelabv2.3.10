...
VM=103; WORK=/tmp/hv2310-adguard; rm -rf "$WORK"; mkdir -p "$WORK"
{
  write_env_header
...
cp /tmp/hv2310-adguard/docker-compose.yml /opt/homelab/adguard-home/docker-compose.yml
cp /tmp/hv2310-adguard/.env /opt/homelab/adguard-home/.env
...
rscp "$WORK" "$VM" /tmp/
rssh "$VM" "sudo /tmp/hv2310-adguard/install.sh"
