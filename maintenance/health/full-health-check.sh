...
echo "🩺 Homelab v2.3.10 health check"
echo
...
rssh 107 'nvidia-smi >/dev/null 2>&1 && echo "nvidia-smi OK" || echo "nvidia-smi missing"; find /dev/disk/by-id -maxdepth 1 -type l | grep -Ei "TOSHIBA_HDWG|HDWG180|HDWG480" | grep -v part | wc -l | awk "{print \"plot disks=\"\$1}"; df -h | grep /mnt/chia-plots || true; grep -R "parallel_decompressor_count" ~/.chia/mainnet/config/config.yaml 2>/dev/null || true; ss -ltnp | grep 55400 || true' || true
