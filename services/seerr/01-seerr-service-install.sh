...
VM=102; WORK=/tmp/hv239-seerr; rm -rf "$WORK"; mkdir -p "$WORK"
...
cp /tmp/hv239-seerr/docker-compose.yml /opt/homelab/seerr/docker-compose.yml
cp /tmp/hv239-seerr/.env /opt/homelab/seerr/.env
...
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv239-seerr/install.sh"
