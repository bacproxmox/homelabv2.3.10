...
prepare_truenas_nextcloud_permissions() {
  ...
  tn_get "system/info" >/tmp/hv239-truenas-info.json || {
  ...
  tn_get "sharing/nfs" >/tmp/hv239-truenas-nfs.json || echo "[]" >/tmp/hv239-truenas-nfs.json
  nfs_id="$(python3 - <<'PY'
import json
from pathlib import Path
try:
    data=json.loads(Path('/tmp/hv239-truenas-nfs.json').read_text() or '[]')
except Exception:
    data=[]
for item in data:
...
cp /tmp/hv239-nextcloud/docker-compose.yml /opt/homelab/nextcloud/docker-compose.yml
cp /tmp/hv239-nextcloud/.env /opt/homelab/nextcloud/.env
...
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv239-nextcloud/install.sh"
